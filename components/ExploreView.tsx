    ...
    return (
        <div className="space-y-8 animate-fade-in text-[var(--text-color)]">
             <div className="p-10 rounded-xl bg-[var(--accent-color)] text-white animate-fade-in-up">
                <h1 className="text-4xl font-extrabold">{t.explore_title}</h1>
                <p className="mt-2 text-lg opacity-90">{t.explore_subtitle}</p>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-20 px-6 bg-[var(--panel-bg)] border border-[var(--border-color)] rounded-xl">
                    <ExploreIcon />
                    <h3 className="mt-4 text-xl font-medium">{t.explore_empty_title}</h3>
                    <p className="mt-1 text-[var(--text-color-light)]">{t.explore_empty_subtitle}</p>
                </div>
            )}
        </div>
    );
